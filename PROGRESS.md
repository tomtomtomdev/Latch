...
| 13 | Menu-bar companion (mini mode) | ✅ Done | §8 | Native `MenuBarExtra(.window)` dropdown over the whole fleet. Fleet-wide surface on `MainWindowModel` (`monitoringSummary`, `allPaused`, `pauseAll`/`resumeAll`, `recentDetections` ≤3) + at-a-glance accessors on `VitalsModel` (`health`/`issueCount`/`statusSummary`/`vitalsLine`) — pure folds, 10 TDD tests. `MenuBarView` (handoff dropdown: header count + green dot, per-target rows icon·vitals·health-status, RECENT DETECTIONS, `Pause all`/`Resume all` + `Open Latch`). Model lifted to `LatchApp` and shared by both scenes; poll loop now polls **all** attached streams (not just selected) so companion health is honest. `TargetRow` de-duplicated onto the new accessors |
...
| 2026-07-08 | Slice 13 poll loop polls **all** attached streams, not just the selected one | The menu-bar companion's whole point is glanceable health across the fleet; polling only the selected target would leave background targets' health/detections stale and dishonest. The `.task` keys on stream count so a newly-attached target starts polling; a paused stream's `poll()` is a no-op (SPEC §8) |
| 2026-07-08 | Companion `recentDetections` = each stream's newest-first feed concatenated in attach order, capped 3 (not globally time-ordered) | The Domain is clock-free (SPEC §4) — there is no wall-clock key to interleave detections across streams by recency. Single-target (the common case) is exactly that stream's top-3; multi-target falls back to attach order. A global sequence would be scope creep into slice-12's `DetectionLog`; revisit if multi-target ordering matters |
| 2026-07-08 | Native `MenuBarExtra(.window)` for the companion, model lifted to `LatchApp` | Verified against the macos-development skill: `MenuBarExtra` is the sanctioned SwiftUI status-item scene; `.window` style gives the popover-like panel the handoff specifies. Lifting the `@Observable` `MainWindowModel` to the `App` lets one instance back both scenes with no AppKit `NSStatusItem` glue; `Open Latch` bridges `NSApp.activate()` via a closure so `MenuBarView` stays AppKit-free |
| 2026-07-14 | Attach-sheet filter uses a plain `TextField`, not `.searchable` | Reproduced live: `.searchable` bare on a `List` inside a fixed-frame `.sheet` (no `NavigationStack`) drives an unbounded "Update Constraints in Window" loop that throws `NSGenericException` (5 exceptions in the unified log; 0 after removal). A `TextField` bound to the same `picker.searchText` keeps the filter without the layout feedback and suits the compact modal. Diagnosis was runtime-observed (log stream + CGWindowList), not unit-testable — Presentation-layer AppKit/SwiftUI behavior |
| 2026-07-14 | One `.sheet(item:)` enum instead of two stacked `.sheet(isPresented:)` | Two presentation bools on one view made SwiftUI observe them as `Pair<Bool, Bool>` and re-run its onChange "multiple times per frame" (seen in the crash log). A single enum-driven sheet is the idiomatic single-source-of-truth and removes that churn |
| 2026-07-14 | `WindowGroup.defaultLaunchBehavior(.presented)` (macOS 15+) | With a `MenuBarExtra` also in the scene, the app could launch with the main window created but never ordered on-screen (only the menu-bar item), so it looked unresponsive on first open. `.presented` forces the window to show at launch. API verified `@available(macOS 15.0)` against the SDK swiftinterface; deployment target is 15.6 |
...
## Changelog
- 2026-07-14 — **Fix: first-open crash + unresponsive window.** The reported "freezing on first
  open" was a fatal `NSGenericException`: `.searchable` bare on the attach sheet's `List` (fixed
  frame, no `NavigationStack`) looped AppKit's "Update Constraints in Window" pass until it threw.
  Reproduced live (forced the sheet open, captured `log stream`): 5 exceptions with `.searchable`,
  **0** after replacing it with a `TextField` filter over the same `picker.searchText`. Also
  consolidated the window's two stacked `.sheet(isPresented:)` into one `.sheet(item:)` enum
  (killed the `onChange(of: Pair<Bool, Bool>) … multiple times per frame` churn) and added
  `WindowGroup.defaultLaunchBehavior(.presented)` so the main window reliably presents on launch
  when the `MenuBarExtra` is also in the scene. Presentation-layer only; 99/99 unit tests green,
  zero new warnings. Verified by launching and observing the unified log, not unit tests.
...
- 2026-07-08 — **Slice 13 (Menu-bar companion / mini mode) landed.** Added the glanceable
  **menu-bar dropdown** over all attached targets via a native `MenuBarExtra` (`.window` style),
  ...
  `MainWindowModel` (a pure-fold extension): `monitoringSummary` (pluralized count), `allPaused` +
  `pauseAll()`/`resumeAll()` (toggle every stream's poller), `recentDetections` (≤3 across targets,
  newest-first per stream). New at-a-glance accessors on `VitalsModel`: `health`/`issueCount`/
  `statusSummary` (`Healthy`/`1 issue`/`N issues`) + `vitalsLine` (`CPU 61% · 540 MB · 12.0 MB/s`,
  honest `—` before the first sample). Presentation: `MenuBarView` reproduces the handoff dropdown
  (18px teal Latch mark + count + green pulse dot header; per-target rows = icon · name · vitals
  line · health dot + status in the health color; RECENT DETECTIONS section tagged live/deep;
  footer `Pause all`/`Resume all` + teal `Open Latch`). `LatchApp` now owns the model and hosts both
  scenes; `MainWindowView` takes the injected model and its poll loop **polls the whole fleet** at
  ~1 Hz (keyed on stream count) so background targets' health is real in the companion, not stale.
  Refactor on green: `SidebarView.TargetRow` de-duplicated onto the new `stream.health`/`issueCount`
  accessors (Fowler: Substitute Algorithm). TDD red-first (10 new app tests, `MenuBarCompanionTests`):
  pause/resume toggles every stream · `allPaused` false when empty · recent-detections reflect the
  slice-12 feed + cap at 3 + empty by default · `monitoringSummary` pluralization · `statusSummary`
  healthy vs active-alert · `vitalsLine` summary + placeholder. Full suite green (app + UI + 97 kit
  tests), clean build, zero new warnings. **Deferred (unchanged):** the `SessionReport` save/export
  trigger and the iOS device rows / ineligibility copy in the sidebar remain deferred to a cleanup
  pass — PLAN slice 13 is the menu-bar companion only, and those were never part of its scope. (PLAN slice 13; SPEC §8)
