...
extension SignalKind {
    ...
    /// Signals Latch can monitor from the live polling loop today. The others need a
    /// deep diagnostic run (zombies, hitches) or a source from a later slice (network,
    /// energy) and stay `unavailable` until then — never faked. (SPEC §1)
    var hasLiveIndicator: Bool {
        self == .cpuSpike || self == .memoryLeak
    }
}
